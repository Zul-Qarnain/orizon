...
function validateSchedule(hours, battery, directives, hourlyPlan, totalGridKwh, totalCostBdt, peakGridKwh, tolerance = 0.05) {
    const errors = [];
    ...
    for (let h = 0; h < 24; h++) {
        ...
        if (Math.abs(supply - demandAndCharge) > tolerance) {
            errors.push(`Hour ${h}: energy balance violated. Supply=${supply.toFixed(2)}, Demand+Charge=${demandAndCharge.toFixed(2)}.`);
        }
        // 2. Solar usage cap
        if (solarUsed > effectiveSolar[h] + tolerance) {
            errors.push(`Hour ${h}: solar_used_kwh (${solarUsed}) exceeds effective solar limit (${effectiveSolar[h].toFixed(2)}).`);
        }
        ...
        if (Math.abs(energyAfter - expectedEnergyAfter) > tolerance) {
            errors.push(`Hour ${h}: battery state transition error. Expected ${expectedEnergyAfter.toFixed(2)}, got ${energyAfter.toFixed(2)}.`);
        }
        // 4. Battery bounds
        if (energyAfter < activeMinEnergy[h] - tolerance) {
            errors.push(`Hour ${h}: battery energy (${energyAfter}) below active reserve level (${activeMinEnergy[h]}).`);
        }
        if (energyAfter > battery.capacity_kwh + tolerance) {
            errors.push(`Hour ${h}: battery energy (${energyAfter}) exceeds capacity (${battery.capacity_kwh}).`);
        }
        // 5. Rate limits
        if (chargeKwh > maxChargeRate[h] + tolerance) {
            errors.push(`Hour ${h}: charge rate (${chargeKwh}) exceeds limit (${maxChargeRate[h]}).`);
        }
        if (dischargeKwh > maxDischargeRate[h] + tolerance) {
            errors.push(`Hour ${h}: discharge rate (${dischargeKwh}) exceeds limit (${maxDischargeRate[h]}).`);
        }
        // 6. Max grid window
        if (grid > maxGridCap[h] + tolerance) {
            errors.push(`Hour ${h}: grid import (${grid}) exceeds max grid cap (${maxGridCap[h]}).`);
        }
        calcTotalGrid += grid;
    ...
    if (Math.abs(finalEnergy - battery.initial_energy_kwh) > tolerance) {
        errors.push(`End-of-day neutrality violated. Expected final energy ${battery.initial_energy_kwh}, got ${finalEnergy}.`);
    }
    // 8. Totals check
    if (Math.abs(totalGridKwh - calcTotalGrid) > tolerance) {
        errors.push(`total_grid_kwh mismatch: reported ${totalGridKwh}, calculated ${calcTotalGrid.toFixed(2)}.`);
    }
    if (Math.abs(totalCostBdt - calcTotalCost) > tolerance) {
        errors.push(`total_cost_bdt mismatch: reported ${totalCostBdt}, calculated ${calcTotalCost.toFixed(2)}.`);
    }
    if (Math.abs(peakGridKwh - calcPeakGrid) > tolerance) {
        errors.push(`peak_grid_kwh mismatch: reported ${peakGridKwh}, calculated ${calcPeakGrid.toFixed(2)}.`);
    }
    return {
